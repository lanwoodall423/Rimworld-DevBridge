...
## Multi-agent identities and shared runtime work

Use the canonical client identity fields deliberately:

- `agentId` names the logical agent and is not an authority token.
- `clientInstanceId` remains stable across a client reconnect and is kept outside the
  RimWorld user root when generated by `devbridge.ps1`.
- `connectionSessionId` changes for a new transport connection.
- `correlationId` is unique to a request attempt.
- `participantId` is the caller's durable membership in one shared operation.

Never reuse another participant's ID or treat a matching label as permission. The
server binds authorization, leases, coordinator ownership, and quotas independently.
Diagnostics redact credentials and sanitize identity/path values.

Shared activation, restart, readiness, save/load, adapter reload, deployment, and
verification work coalesces only on the canonical compatibility key. Correlation IDs,
goal IDs, and participant IDs are intentionally excluded from that key. Record the
returned `operationId`, `compatibilityKey`, `runtimeSlotId`, `deploymentId`, artifact
and loaded fingerprints, PID/start identity, session, lifecycle generation, progress
sequence, `capacityState`, and `keepRunning` before reconnecting or waiting. Detach or
cancel only your own participation. A final detach follows the operation's persisted
abandonment policy and never implicitly kills or adopts a process.

Runtime slots isolate user/config/mod, save, log, IPC, evidence, coordinator, and
deployment paths. Compatible requests may share a slot; incompatible requests wait
fairly or use an isolated slot. Attached/manual processes are always operator-owned.
Treat `attached_live_process_requires_operator`, artifact/deployment mismatch, stale
